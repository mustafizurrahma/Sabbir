...
/* ============================================================== */
(function ($) {
  "use strict"; // Init Metronal

  var metronal = {}; // Init Main Content
  ...
  metronal.portfolioFilter = {
    ...
    init: function init() {
      ...
      metronal.portfolioFilter.container.imagesLoaded(function () {
        ...
        metronal.portfolioFilter.container.isotope('layout'); // Filter items when the button is clicked

        $('#portfolio .portfolio-filter ul li').on('click', 'a', function () {
          // Remove the current class from the previous element
          $('#portfolio .portfolio-filter ul li .current').removeClass('current'); // Add the current class to the button clicked

          $(this).addClass('current'); // Data filter
  ...
  metronal.setSkillProgress = function () {
    ...
    for (var i = 0; i < skill.length; i++) {
      if (skill.eq(i).find('.percentage')[0].textContent == '100%') {
        skill.eq(i).find('.progress-wrapper .progress').css({
  ...
  metronal.processContactForm = function () {
    ...
    var failFunc = function failFunc(jqXHR, textStatus, errorThrown) {
      if (jqXHR.status === 400) {
  ...
  $(document).ready(function () {
    metronal.dynamicPage(undefined, window.location.hash), metronal.replaceVHeight(), metronal.portfolioFilter.init(), metronal.useMagnificPopup(), metronal.setSkillProgress(), metronal.progressAnimation(), metronal.useTypeIt(), metronal.processContactForm();
  });
})(jQuery);